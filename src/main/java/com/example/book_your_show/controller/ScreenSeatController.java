...
public class ScreenSeatController {
    ...
    public ResponseEntity addScreenSeats(@RequestParam String theatreCode,
                                        ...
                                        @RequestBody List<ScreenSeatRequest> screenSeatRequestList,
                                        @RequestParam List<String>pdSeatList){
        try {
            return new ResponseEntity<>(screenSeatService.addScreenSeats(theatreCode, screenNumber, screenSeatRequestList, pdSeatList), HttpStatus.CREATED);
        } catch (Exception e) {
